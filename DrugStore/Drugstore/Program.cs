...
namespace Drugstore
{
    ...
    public class Store
    {
            ...
            using (var reader = new StreamReader($"{this.DATASET_LOCATION_FOLDER}/effects.txt"))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    var effects = temp[1].Split(';').Select(x => x.Trim()).Select(eff => {
                        var tempp = eff.Trim(')', '(', ' ').Split(',').Select(x => x.Trim()).ToArray();
                        return (drug: tempp[0], effname: tempp[1]);
                    }).ToArray();
            ...
            using (var reader = new StreamReader($"{this.DATASET_LOCATION_FOLDER}/alergies.txt"))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    var allergies = temp[1].Split(';').Select(x => x.Trim()).Select(al => {
                        var tempp = al.Trim(')', '(', ' ').Split(',').Select(x => x.Trim()).ToArray();
                        return (drug: tempp[0], altype: tempp[1]);
                    }).ToArray();
        ...
        {
            Random rand = new Random();
            return this.Diseases.Keys.ElementAt(rand.Next(0, Math.Min(100, this.Diseases.Count)));
        }

        private string RandomEffect()
        {
            Random rand = new Random();
            var effects = this.Drugs.Values.Take(Math.Min(100, this.Drugs.Count)).SelectMany(x => x.Effects.Select(e => e.effect)).ToArray();
            if (effects.Count() == 0)
                return $"Eff_{RandomString()}";
            else
                return effects.ElementAt(rand.Next(0, effects.Count()));
        }
